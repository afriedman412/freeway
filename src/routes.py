...
def get_ies(date: str = None):
    ...
    if not date:
        ...
        date = get_today()
        custom_date = False
    date_ies = query_table(f"select * from {IE_TABLE} where dissemination_date='{date}'")
    if not date_ies:
        ie_message = f"No Independent Expenditures for {date}." if custom_date else f"No Independent Expenditures for today ({date})."
        logger.debug("No IEs for selected date, getting 10 most recent")
        date_ies = query_table(f"select * from {IE_TABLE} order by dissemination_date desc limit 10")
        returned_transactions = False
    ie_message = f"Independent Expenditures for {date}." if custom_date else f"New Independent Expenditures for today ({date})!!"
